	...
	const timer = setInterval(function () {
		...
		if (value >= progressBar.getTotal()) {
			...
			fse.mkdtemp(path.join(os.tmpdir(), "exzo-"), (err, folder) => {
				if (err) throw err;
				execSync(
					`git clone --depth 1 ${"https://github.com/raebeatrose4/create-nexis-dapp"} ${folder}`,
					{ stdio: "pipe" }
				);
...
`)
	...
	console.log("\n");
	console.log("🪸 Welcome to the create-nexis-dapp wizard 🪸");
